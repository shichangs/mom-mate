...
- Notes 模块：仅开发调试使用（对普通用户不可见，且睡眠页不提供用户入口）。
...
2. 饮食记录
- 快速新增饮食记录（餐次 + 食物）
- 打开“添加饮食记录”时根据当前时间自动预选早餐/午餐/晚餐
- 餐次包含早餐、午餐、晚餐、加餐、奶、水（支持记录喝水量 ml）
- 选择“水”时提供快捷档位（50/100/200/500ml）与自定义输入
- 按餐次筛选记录（筛选栏采用多列换行布局，默认可直接看到“水”“奶”等全部餐次）
- 今日饮食摘要（次数/分类，不包含饮水次数）
- 今日饮水摘要（目标饮水量、当日累计饮水量、进度）
- 妈妈食谱入口采用有色卡片样式，和摘要卡片保持一致视觉层级
- 支持删除记录
...
- 饮食统计（周/月/年频次、类型分布、摘要、每日喝水量）
- 图形化展示（柱状/环形）
- 睡眠统计口径统一为“总时长 + 日均时长 + 日均次数”；年视图按“月日均睡眠时长”展示
- 饮食统计摘要文案统一为“总进食次数 + 日均进食次数 + 日均喝水量”
- 饮食类型分布需包含饮水（“水”）分类
- 饮食类型分布需提供可读图例，明确展示“水”分类计数
...
2. 内置文档/笔记模块（内部）
- 仅供开发调试查看与编辑 Markdown 文档，不作为用户可见功能入口
...
- MealRecord：时间、餐次、食物、食量、喝水量（ml，可选）、备注
...
- 历史记录可编辑时间并保存生效
- 历史记录列表样式与首页“最近记录”保持一致（时长、时间线、日期标签视觉统一）
- 首页“最近记录”点击单条可直接进入编辑页并保存修改

2. 饮食
- 可新增一条饮食记录并在列表立即可见
- 打开“添加饮食”时，默认餐次会随当前时段自动选择早餐/午餐/晚餐
- 餐次筛选后列表结果正确
- 今日饮食摘要中的次数不包含饮水记录
- 新增“水”记录后可输入并保存喝水量（ml）
- 今日饮水支持设置目标并显示进度
- 妈妈食谱入口以有色卡片展示，且与上下模块间距保持紧凑
...
- 饮食统计可展示每日喝水量与日均喝水量
- 饮食类型分布图包含“水”分类并与其他餐次同口径展示
- 统计周期包含周/月/年，且未来周期不可继续前进

5. 设置
- 字体缩放调整后，四个主 Tab 页面文字同步变化
- 全局界面文案与日期时间选择器按中文展示（含轮盘日期/时间选择器）
- 支持按模块选择清空数据（睡眠/饮食/里程碑），且确认按钮需倒计时 5 秒后才可点击
- 设置页展示当前 App 版本号（格式：`主版本.次版本.小版本 (构建号)`）
- 设置页展示“本次更新”简洁变更信息，内容与当前版本对应
...
- 饮食：`meal_type`、`food_count`、`water_ml`
- 里程碑：`category`
- 统计：`statistics_mode`（sleep/meal）

### 12.4 统计口径速查表（实现对齐）

- 睡眠总睡眠时长：所选周期内有效睡眠记录时长总和（仅统计 `wakeTime != nil`）。
- 睡眠日均睡眠时长：`总睡眠时长 / 周期天数`。
- 睡眠日均睡眠次数：`睡眠记录次数 / 周期天数`。
- 饮食总进食次数：所选周期内非饮水记录总条数（`mealType != 水`）。
- 饮食日均进食次数：`总进食次数 / 周期天数`。
- 饮食日均喝水量：`周期内饮水总量（ml） / 周期天数`。
- 饮食类型分布：所选周期内各餐次出现次数（包含 `水`）。
...
- 餐次为必填，食物可为空。
- 打开“添加饮食记录”时，默认餐次按当前时间自动选择：早餐（05:00-10:59）、午餐（11:00-16:59）、晚餐（其余时段）。
- `mealType = 水` 时，喝水量（ml）必须为正整数。
- `mealType = 水` 时，应提供快捷档位 50/100/200/500ml 与自定义输入。
- 每日饮水目标必须为正整数（ml）。
